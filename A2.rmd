...
incage_hist <- gssdata %>% 
  ...
  geom_histogram() +
  labs(title = "Age Distribution of Each Income Level", x = "Income Level", y = "Count", fill = "Income Level", caption = "Figure 1") +
  facet_grid(~income_level) +
  theme_minimal() + theme(plot.title = element_text(hjust = 0.5)) + 
  geom_vline(mapping = aes(xintercept=mean1), colour = "red", linetype = "dashed") + 
  geom_vline(mapping = aes(xintercept=mean2), colour = "blue", linetype = "dashed") + scale_fill_brewer(palette="Accent") + scale_color_brewer(palette="Accent") + theme(plot.caption = element_text(hjust = 0.5))
...
incfeel_bar <- gssdata %>% 
  ...
  labs(title = "Satisfaction towards Life of Each Income Group", x = "Income Level", y = "Proportion", fill = "Satisfaction towards Life", color = "Satisfaction towards Life", caption = "Figure 2") +
  facet_grid(~income_level) + scale_y_continuous(labels = scales::percent) +
  geom_text(aes( label = scales::percent(..prop..),
                   y= ..prop.. ), stat= "count", vjust = -.5, size = 1.5) +
  theme_minimal() + theme(plot.title = element_text(hjust = 0.5)) + theme(plot.caption = element_text(hjust = 0.5))

 # Table
table2 <- kable(gssdata %>% group_by(income_level) %>% summarize(Mean = round(mean(feelings_life), 4), Q1 = quantile(feelings_life, 0.25), Median = median(feelings_life), Q3 = quantile(feelings_life, 0.75), IQR = IQR(feelings_life), Outliers = sum(feelings_life < quantile(feelings_life, 0.25) - IQR(feelings_life)*1.5 | feelings_life > quantile(feelings_life, 0.75) + IQR(feelings_life)*1.5), "Standard Deviation" = round(sd(feelings_life), 4), Total = n()), caption = "Table 2: Statistics of Satisfaction towards Life (by Income Group)")
...
# Table
table3 <- kable(gssdata %>% group_by(sex) %>% summarize("High Income" = sum(income_level == "High"), "Low Income" = sum(income_level == "Low"), "High Income (%)" = round(sum(income_level == "High")/n(), 2), "Low Income (%)" = round(sum(income_level == "Low")/n(), 2), "Total" = n()), caption = "Table 3: Statistics of Sex (by Income Group)")
